...
uniform float u_red; uniform float u_blue; uniform float u_green;
void main() { gl_FragColor = vec4(vec3(u_red, u_green, u_blue), 1.0); }
`;
...
export default function VisualizerScene({ frequency }: VisualizerSceneProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (freq > 0) {
        uniforms.u_frequency.value = freq * 1.5;
      } else {
        const time = clock.getElapsedTime();
        uniforms.u_frequency.value = Math.max(0, 30 + Math.sin(time * 2) * 15 + Math.sin(time * 5) * 10 + Math.cos(time * 3) * 5);
      }
